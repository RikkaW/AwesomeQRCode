...
open class AwesomeQrRenderer {
     ...
     */
    private val BYTE_EPT = 0x0.toByte()
    private val BYTE_DTA = 0x1.toByte()
    private val BYTE_POS = 0x2.toByte()
    private val BYTE_AGN = 0x3.toByte()
    private val BYTE_TMG = 0x4.toByte()
    private val BYTE_PTC = 0x5.toByte()

    @Throws(Exception::class)
    open fun render(renderOptions: RenderOption): RenderResult {
        if (renderOptions.background is BlendBackground && renderOptions.background!!.bitmap != null) {
            ...
            val scaledBoundingRects =
                scaleImageBoundingRectByClippingRect(background.bitmap!!, renderOptions.size, background.clippingRect)
            val fullRendered = Bitmap.createScaledBitmap(
                background.bitmap!!,
        ...
        } else if (renderOptions.background is GifBackground) {
    ...
    open fun renderAsync(
        renderOptions: RenderOption,
        resultCallback: ((RenderResult) -> Unit)?,
     ...
     */
    @Throws(Exception::class)
    protected fun renderFrame(renderOptions: RenderOption, backgroundFrame: Bitmap?): Bitmap {
        ...
        val unscaledFullRenderedBitmap =
            Bitmap.createBitmap(unscaledFullRenderSize, unscaledFullRenderSize, Bitmap.Config.ARGB_8888)
        ...
        if (backgroundFrame != null && renderOptions.background != null) {
            paint.alpha = (255 * renderOptions.background!!.alpha).roundToInt()
        ...
        if (renderOptions.logo != null && renderOptions.logo!!.bitmap != null) {
            ...
            logoCanvas.drawRoundRect(logoRectF, logo.borderRadius.toFloat(), logo.borderRadius.toFloat(), logoPaint)
            unscaledCanvas.drawBitmap(
                logoOpt, (0.5 * (unscaledFullRenderedBitmap.width - logoOpt.width)).toInt().toFloat(),
        ...
        if (renderOptions.background is BlendBackground) {
            renderedResultBitmap =
                Bitmap.createBitmap(renderedScaledBitmap.width, renderedScaledBitmap.height, Bitmap.Config.ARGB_8888)
            ...
            finalRenderedCanvas.drawRoundRect(
                finalClippingRectF,
                (renderOptions.background as BlendBackground).borderRadius.toFloat(),
                (renderOptions.background as BlendBackground).borderRadius.toFloat(),
                finalClippingPaint
            )
    ...
    private fun getDominantColor(bitmap: Bitmap): Int {
        ...
        if (c == 0) {
            ...
            red = (red / c).coerceIn(0, 0xff)
            green = (green / c).coerceIn(0, 0xff)
            blue = (blue / c).coerceIn(0, 0xff)
